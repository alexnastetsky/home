...
test('smoke test - the shell landing page links to both apps', async ({ page }) => {
  await page.goto('/');

  await expect(page.getByRole('heading', { name: 'Home' })).toBeVisible();
  await expect(page.getByRole('link', { name: /Todos/ })).toHaveAttribute('href', '/todolist');
  await expect(page.getByRole('link', { name: /World Cup Pool/ })).toHaveAttribute('href', '/worldcup');
});
...
test('smoke test - standings page gated until lock or visible', async ({ page }) => {
  await page.goto('/worldcup/standings');
...
test('smoke test - my picks page shows the predictions form', async ({ page }) => {
  await page.goto('/worldcup/my-picks');
